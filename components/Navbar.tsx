...
const Navbar = () => {
    ...
    const [logoSrc, setLogoSrc] = useState("/Home/logo.png"); // New state variable for logo source
    ...
    useEffect(() => {
        ...
        const handleScroll = () => { // New scroll event handler
            if (window.scrollY > 50) {
                setLogoSrc("/Home/Translead Logo 1.png"); // Change to your other logo path
            } else {
                setLogoSrc("/Home/logo.png");
            }
        };
            ...
            <nav className="bg-white border-gray-200 border-b shadow-lg p-8 pt-4 md:pt-8 md:pb-14 pb-12  min-w-full fixed z-20 h-12 md:h-8">
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto">
                        <Link href='/' className="flex items-center">
                            <img src={logoSrc} className="h-8 mr-3" alt="Flowbite Logo" /> {/* Use logoSrc state variable */}
                        </Link>
                    <button
                        onClick={() => {
                            setIsOpen(!isOpen);
